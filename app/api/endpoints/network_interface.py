...
from core.models import NetworkInterface
...
@router.get("/network/{node}", response_model=List[NetworkInterface])
async def get_node_network(node: str):
    """
    ...
    for n in inventory.get_hosts():
        if str(n) == node:
            ...
            for _, network_data in enumerate(host['network_interfaces']):
                ...
                node_network_interfaces.append(network_interface)

    return node_network_interfaces
...
async def get_network_mac(network: str):
    """
